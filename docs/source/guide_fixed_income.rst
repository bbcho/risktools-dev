...
Under discrete compounding with an annual interest rate :math:`r` and
:math:`n` compounding periods, the relationship between present value (PV) and
future value (FV) is:
...
The quantity :math:`d(n) = 1 / (1 + r)^{n}` is called the **discount factor**
for period :math:`n`. Discount factors are always between zero and one for
...
When interest is compounded :math:`m` times per year at a nominal annual rate
:math:`r`, the per-period rate is :math:`r/m` and the number of periods over
:math:`T` years is :math:`n = m \cdot T`:
...
As the compounding frequency :math:`m` increases without bound, we arrive at
...
where :math:`t` is measured in years. Continuous compounding is the standard
...
Consider a bond with face value :math:`F`, annual coupon rate :math:`c`,
:math:`m` coupon payments per year, :math:`n = m \cdot T` total periods, and
yield to maturity :math:`y`. The coupon payment each period is
:math:`C = c \cdot F / m`. The price is the sum of the present values of all
cash flows:
...
The relationship between the coupon rate :math:`c` and the yield to maturity
:math:`y` determines whether a bond trades at, above, or below its face value:

- **Par bond** (:math:`c = y`): The bond's price equals its face value,
  :math:`P = F`.
- **Premium bond** (:math:`c > y`): The coupon rate exceeds the required yield,
  so investors pay more than face value, :math:`P > F`.
- **Discount bond** (:math:`c < y`): The coupon rate is below the required
  yield, so the bond sells below face value, :math:`P < F`.
...
Let :math:`t_i` denote the time (in years) at which the :math:`i`-th cash flow
:math:`CF_i` is received, and let :math:`PV(CF_i)` denote its present value.
...
This is a time-weighted average: each future payment date :math:`t_i` is
...
- **Perpetuity.** For a perpetuity (infinite stream of coupons, no principal
  repayment), duration equals :math:`(1 + y) / y`.
...
The term :math:`D_{\text{Mac}} / (1 + y/m)` is called **modified duration**:
...
Let :math:`CF_0` (typically negative) be the initial investment cost, and let
:math:`CF_1, CF_2, \ldots, CF_n` be the subsequent cash flows at times
:math:`t_1, t_2, \ldots, t_n`. With a discount factor :math:`d(t_i)` for each
...
Under continuous compounding with a flat yield :math:`r`, this becomes:

.. math::

   NPV = CF_0 + \sum_{i=1}^{n} CF_i \cdot e^{-r \, t_i}

The decision rule is straightforward: accept the project if :math:`NPV > 0`;
reject if :math:`NPV < 0`. A zero NPV means the project exactly earns the
...
where :math:`y(t)` is the zero rate for maturity :math:`t`. The ``npv()``
...
where :math:`N` is the notional, :math:`r_{\text{fix}}` is the fixed rate, and
:math:`\Delta t_i` is the number of days in period :math:`i`.
...
where :math:`d(t)` denotes the discount factor at time :math:`t`. The floating
payment for period :math:`[t_i, t_{i+1}]` is then:
...
- ``"rec"`` means the user **receives fixed** and pays floating. The value is
  :math:`PV(\text{Fixed}) - PV(\text{Float})`.
...
where :math:`F_1` is the price of the first (expiring) contract and :math:`F_2`
...
5. The swap price on each date is:
   :math:`P_{\text{swap}} = w_1 \cdot F_1 + w_2 \cdot F_2`.
