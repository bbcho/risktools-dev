...
functions in :mod:`risktools`. It is written for students of quantitative finance
...
.. list-table:: Annualization Scale Factors
   :header-rows: 1
   :widths: 30 20

   * - Frequency
     - Scale
   * - Daily / Business-day (``D``, ``B``)
     - 252
   * - Weekly (``W``)
     - 52
   * - Monthly (``M``, ``ME``, ``MS``)
...
investment. Given an asset price :math:`P_t` at time :math:`t`, there are three
...
The function :func:`risktools.return_cumulative` computes both variants:
...
- :math:`n` is the total number of observed periods, and
- :math:`s` is the annualization scale (the number of periods per year).

The exponent :math:`s/n` converts the cumulative growth factor to an annual
...
where :math:`\bar{R}` is the arithmetic mean of the period returns. This simply
...
:func:`risktools.return_annualized` implements both approaches:
...
of period returns :math:`R_1, R_2, \ldots, R_n`:
...
where :math:`s` is the annualization scale factor. The theoretical justification
...
:math:`s` independent random variables equals :math:`s` times the variance of
one variable:
...
:func:`risktools.sd_annualized` applies this rule:
...
- :math:`R_p` is the portfolio return,
- :math:`R_f` is the risk-free rate, and
- :math:`\sigma_p` is the standard deviation of portfolio returns.
...
:func:`risktools.sharpe_ratio_annualized` computes the annualized version:
...
:math:`\tau` (the Minimum Acceptable Return, or MAR) and compares the
probability-weighted gains above :math:`\tau` to the probability-weighted
losses below it:
...
where :math:`F(r)` is the cumulative distribution function of returns.
...
:func:`risktools.omega_sharpe_ratio` computes this measure:
...
Acceptable Return (MAR)** or target return :math:`\tau`. Common choices include:

- :math:`\tau = 0` (absolute loss avoidance),
- :math:`\tau = R_f` (the risk-free rate), or
- :math:`\tau = \bar{R}` (the mean return, yielding the semi-deviation).
...
By using :math:`n` (the full number of observations) in the denominator rather
...
this choice: ``"full"`` (default) uses :math:`n`; ``"subset"`` uses the count
of below-MAR observations only.

**Semi-deviation** is the special case where :math:`\tau = \bar{R}`:
...
:func:`risktools.downside_deviation`.
...
:func:`risktools.upside_risk` computes these, controlled by the ``stat``
...
Let :math:`W_t` denote the cumulative wealth index at time :math:`t`:
...
The drawdown at time :math:`t` is:

.. math::

   D_t = \frac{W_t}{M_t} - 1

Note that :math:`D_t \le 0` during drawdown periods and :math:`D_t = 0` at new
highs.

For arithmetic (non-compounded) returns, the wealth index is instead
:math:`W_t = 1 + \sum_{i=1}^{t} R_i`.
...
:func:`risktools.drawdowns` computes the drawdown time series, and
:func:`risktools.find_drawdowns` identifies individual drawdown episodes with
...
- :math:`R_a` is the asset return,
- :math:`R_b` is the benchmark (market) return,
- :math:`R_f` is the risk-free rate,
- :math:`\alpha` is the intercept (Jensen's alpha -- the risk-adjusted excess return),
- :math:`\beta` is the slope coefficient, and
- :math:`\epsilon` is the idiosyncratic error term.
...
- :math:`\beta = 1`: the asset moves in lockstep with the benchmark.
- :math:`\beta > 1`: the asset amplifies benchmark movements (higher systematic risk).
- :math:`\beta < 1`: the asset dampens benchmark movements (lower systematic risk).
- :math:`\beta < 0`: the asset moves inversely to the benchmark (a hedge).

Bull Beta and Bear Beta
------------------------

The overall beta may mask asymmetric behavior in up and down markets. **Bull
beta** (:math:`\beta^{+}`) is estimated using only periods where the benchmark
had positive excess returns. **Bear beta** (:math:`\beta^{-}`) is estimated
...
An investor prefers a manager with :math:`\beta^{+} > \beta^{-}`: the manager
captures more of the upside than the downside.

Code Example
------------

:func:`risktools.CAPM_beta` performs the regression using scikit-learn's
``LinearRegression``:
...
The coefficient :math:`\beta_2` captures the incremental response to negative
market returns. A positive :math:`\beta_2` indicates that the manager
...
:func:`risktools.timing_ratio` computes this directly:
...
.. list-table::
   ...
   * - Concept
     ...
     - Section
   * - Cumulative return
     - :func:`~risktools.return_cumulative`
     - 2
   * - Annualized return
     - :func:`~risktools.return_annualized`
     - 3
   * - Excess return
     - :func:`~risktools.return_excess`
     - 5
   * - Annualized standard deviation
     - :func:`~risktools.sd_annualized`
     - 4
   * - Annualized Sharpe Ratio
     - :func:`~risktools.sharpe_ratio_annualized`
     - 5
   * - Omega-Sharpe Ratio
     - :func:`~risktools.omega_sharpe_ratio`
     - 6
   * - Downside deviation
     - :func:`~risktools.downside_deviation`
     - 7
   * - Upside risk / potential
     - :func:`~risktools.upside_risk`
     - 7
   * - Drawdown levels
     - :func:`~risktools.drawdowns`
     - 8
   * - Drawdown episodes
     - :func:`~risktools.find_drawdowns`
     - 8
   * - CAPM beta
     - :func:`~risktools.CAPM_beta`
     - 9
   * - Timing ratio
     - :func:`~risktools.timing_ratio`
     - 10
