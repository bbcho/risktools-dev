...
where :math:`W_t` is a **Wiener process** (standard Brownian motion) satisfying:

1. :math:`W_0 = 0`
2. :math:`W_t - W_s \sim \mathcal{N}(0, t-s)` for :math:`t > s`
3. Increments over non-overlapping intervals are independent

The term :math:`a(X_t, t)\,dt` is the **drift** (deterministic trend), and
:math:`b(X_t, t)\,dW_t` is the **diffusion** (random fluctuation). Different
choices of the functions :math:`a` and :math:`b` yield different stochastic
...
infinitesimal increments with small but finite time steps :math:`\Delta t`.
...
- :math:`\mu` is the drift rate (expected return per unit time),
- :math:`\sigma` is the volatility (annualized standard deviation of returns),
- :math:`W_t` is a standard Wiener process.

The key feature of this SDE is that both the drift and diffusion are
**proportional to the current price** :math:`S_t`. This ensures that prices
...
Applying Ito's lemma to :math:`\ln S_t`, we obtain the exact solution:
...
- :math:`\ln S(t)` is normally distributed, so :math:`S(t)` is
  **log-normally distributed**.
- The expected value is :math:`\mathbb{E}[S(t)] = S(0)\,e^{\mu t}`.
- The term :math:`-\sigma^2/2` is the **Ito correction**, arising from the
...
Rather than discretizing the SDE for :math:`S_t` directly (which would require
multiplicative noise), we discretize the log-price process. Over a small time
step :math:`\Delta t`, the log return is:

.. math::

   \ln S_{t+\Delta t} - \ln S_t = \left(\mu - \tfrac{\sigma^2}{2}\right)\Delta t + \sigma\,\sqrt{\Delta t}\;\varepsilon_t

where :math:`\varepsilon_t \sim \mathcal{N}(0, 1)`. Exponentiating gives the
...
This is precisely the scheme implemented in :func:`~risktools.simGBM`. By
...
- The process is a **martingale** under the risk-neutral measure (when
  :math:`\mu = r`, the risk-free rate).
- The variance of :math:`S(t)` grows without bound as :math:`t \to \infty`.
...
The function :func:`~risktools.simGBM` simulates GBM paths using the
...
- :math:`X_t` is the process value (e.g., a spread level) at time :math:`t`,
- :math:`\mu` is the **long-run mean** to which the process reverts,
- :math:`\theta > 0` is the **mean-reversion speed** (higher values mean
  faster reversion),
- :math:`\sigma` is the volatility,
- :math:`W_t` is a standard Wiener process.

The drift term :math:`\theta(\mu - X_t)` acts as a spring:

- When :math:`X_t > \mu`, the drift is negative, pulling the process down.
- When :math:`X_t < \mu`, the drift is positive, pushing the process up.
- The strength of the pull is proportional to both :math:`\theta` and the
  deviation :math:`|X_t - \mu|`.

.. note::

   Unlike GBM, the diffusion term in the OU process is **additive** (not
   proportional to :math:`X_t`). This means the OU process can theoretically
...
Setting :math:`X_t - \mu = \tfrac{1}{2}(X_0 - \mu)` and solving for :math:`t`:

.. math::

   t_{1/2} = \frac{\ln 2}{\theta}

For example, if :math:`\theta = 2` (per year), the half-life is
:math:`\ln 2 / 2 \approx 0.347` years, or about 87 business days. The
...
distribution**. As :math:`t \to \infty`, the distribution of :math:`X_t`
converges to:
...
1. **The variance is finite** and determined by the ratio
   :math:`\sigma^2 / (2\theta)`. Faster mean reversion (larger :math:`\theta`)
   or lower volatility (smaller :math:`\sigma`) produces a tighter
   distribution around :math:`\mu`.
2. **The process is ergodic**: time averages converge to ensemble averages,
   which justifies estimating :math:`\mu` from a single long time series.

Simulation with ``simOU``
--------------------------

The function :func:`~risktools.simOU` simulates OU paths with support for
...
- :math:`\theta`, :math:`\mu`, :math:`\sigma`, and :math:`W_t` have the same
  interpretation as in the standard OU process,
- :math:`N_t` is a **Poisson process** with intensity :math:`\lambda`
  (the probability of a jump per unit time),
- :math:`J_t` is the **jump size**, drawn from a log-normal distribution with
  mean :math:`\bar{J}` and standard deviation :math:`\sigma_J`,
- the drift is adjusted by :math:`-\lambda\,\bar{J}` to compensate for the
  expected jump contribution, keeping the long-run mean centered at
  :math:`\mu`.

The Poisson process :math:`N_t` generates random arrival times for jumps.
In any infinitesimal interval :math:`[t, t+dt)`:

.. math::

   \Pr(dN_t = 1) = \lambda\,dt, \qquad \Pr(dN_t = 0) = 1 - \lambda\,dt

When a jump occurs (:math:`dN_t = 1`), the process experiences an
instantaneous shift of size :math:`J_t`. Between jumps, the process follows
...
of size :math:`J` occurs at time :math:`t`:

.. math::

   \mu_{\text{eff}}(s) = \mu + J \quad \text{for } s \in [t, t + \texttt{mr\_lag} \cdot \Delta t)

After the lag period, the effective mean reverts to :math:`\mu`, and the
...
The function :func:`~risktools.simOUJ` simulates OU jump-diffusion paths.
...
- ``jump_prob`` -- jump intensity :math:`\lambda` (probability per unit time)
- ``jump_avgsize`` -- mean jump size :math:`\bar{J}` (log-normal mean)
- ``jump_stdv`` -- standard deviation of jump size (log-normal :math:`\sigma_J`)
- ``T`` -- time horizon in years
- ``dt`` -- time step as a fraction of a year
- ``sims`` -- number of Monte Carlo paths
...
to estimate the parameters :math:`\theta`, :math:`\mu`, and :math:`\sigma`.
The function :func:`~risktools.fitOU` provides two estimation methods: **OLS
...
where :math:`\Delta X_t = X_{t+\Delta t} - X_t`. This is a linear regression
of the changes :math:`\Delta X_t` on the levels :math:`X_t`, with:

.. math::

   a = \theta\,\mu\,\Delta t, \qquad b = -\theta\,\Delta t

From the OLS estimates :math:`\hat{a}` and :math:`\hat{b}`:
...
where :math:`\hat{\varepsilon}` denotes the OLS residuals. The OLS method is
...
distribution of :math:`X_{t+\delta}` given :math:`X_t` is:
...
The MLE uses sufficient statistics :math:`S_x`, :math:`S_y`, :math:`S_{xx}`,
:math:`S_{yy}`, and :math:`S_{xy}` computed from the observations to obtain
...
not depend on the choice of :math:`\Delta t` in the same way as OLS.
...
1. **Data frequency and** ``dt``. The ``dt`` parameter must match the actual
   ...
   ``dt=1/252``. Misspecifying ``dt`` will scale :math:`\hat{\theta}` and
   :math:`\hat{\sigma}` incorrectly.
...
2. **Sample size.** Mean-reversion parameters are notoriously difficult to
   ...
   span at least several half-lives (:math:`T \gg \ln 2 / \theta`) to obtain
   reliable estimates. With daily data and :math:`\theta \approx 2`, this
   means at least 2--3 years of data.

3. **Stationarity.** The OU model assumes the process is stationary (or at
   least covariance-stationary). If the long-run mean :math:`\mu` is shifting
...
4. **OLS vs. MLE.** For large samples with small :math:`\Delta t`, both
...
.. list-table::
   ...
   * - Process
     ...
     - Use Case
   * - GBM
     - :math:`dS = \mu S\,dt + \sigma S\,dW`
     - No
     - No
     - Stock prices, short-horizon commodity
   * - OU
     - :math:`dX = \theta(\mu - X)\,dt + \sigma\,dW`
     - Yes
     - No
     - Spreads, basis, calendar rolls
   * - OUJ
     - :math:`dX = \theta(\mu - X)\,dt + \sigma\,dW + J\,dN`
     - Yes
     - Yes
     - Energy spreads with supply shocks
