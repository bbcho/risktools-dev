...
def return_annualized(r, scale=None, geometric=True):
    ...
    """
    if ~(isinstance(r, pd.DataFrame) | isinstance(r,pd.Series)):
        raise ValueError("r must be a pandas Series or DataFrame")
    ...
    if isinstance(r.index, pd.DatetimeIndex):
        if scale is None:
            if (r.index.freq == 'D') | (r.index.freq == 'B'):
                ...
                raise ValueError("parameter r's index must be a datetime index with freq 'D','W','M','Q' or 'Y'")
    else:
        raise ValueError("parameter r's index must be a datetime index with freq 'D','W','M','Q' or 'Y'")
...
def upside_risk(R, MAR=0, method='full', stat='risk'):
    ...
    *args :
    ...
    if stat == 'risk':
        result = r.sub(MAR).pow(2).div(length).sum().apply(np.sqrt)
    elif stat == 'variance':
...
def downside_deviation(R, MAR=0, method='full', potential=False):
    ...
    *args :
    ...
    if potential:
        result = -r.add(MAR).div(length).sum()
    else:
        result = -r.add(MAR).pow(2).div(length).sum().apply(np.sqrt)    
